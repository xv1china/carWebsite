...
<head>
  ...
  <title><?= t('blog_page_title', 'ბლოგი - კკკ & კომპანია | Blog') ?></title>
  <meta name="description" content="<?= t('blog_meta_desc', 'სატვირთო მანქანების, ლოჯისტიკისა და გადაზიდვების შესახებ სიახლეები და სტატიები') ?>">
  ...
  <style>
    ...
    .blog-card-modern { height: 100%; }
    .blog-card-modern .row { height: 100%; }
    ...
    .blog-card-modern .card-body { min-height: 220px; }
...
<body class="bg-light">
    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav ms-auto gap-3">
          ...
          <li class="nav-item"><a class="nav-link active" href="blog.php"><?= t('nav_blog', 'ბლოგი') ?></a></li>
  ...
  <section class="py-5 bg-primary text-white text-center">
    <div class="container">
      <h1 class="display-4 fw-bold mb-3"><?= t('blog_h1', 'ბლოგი') ?></h1>
      <p class="lead"><?= t('blog_header_text', 'სიახლეები და სტატიები სატვირთო მანქანების, ლოჯისტიკისა და გადაზიდვების შესახებ') ?></p>

      <?php if ($id <= 0): ?>
        <div class="row justify-content-center mt-4">
          <div class="col-lg-6">
            <form method="get" class="d-flex gap-2">
              <input name="q" class="form-control" placeholder="<?= t('search_placeholder', 'ძებნა...') ?>" value="<?= htmlspecialchars($q) ?>">
              <button class="btn btn-light fw-bold"><?= t('search_btn', 'ძებნა') ?></button>
            </form>
          </div>
        </div>
      <?php endif; ?>
  ...
  <section class="py-5">
    <div class="container">
      ...
      <?php if ($id > 0): ?>
        ...
        <a href="blog.php" class="btn btn-outline-secondary mb-4"><?= t('back_btn', '← უკან') ?></a>
      ...
      <?php else: ?>
        ...
        <div class="row g-4">
          ...
          <?php if (!$posts): ?>
            <div class="col-12">
              <div class="alert alert-warning"><?= t('not_found', 'ვერ მოიძებნა.') ?></div>
            </div>
          <?php endif; ?>
            ...
            <ul class="pagination justify-content-center">
              <?php for ($i=1; $i<=$pages; $i++): ?>
                <li class="page-item <?= ($i===$page) ? "active" : "" ?>">
                  <a class="page-link" href="?q=<?= urlencode($q) ?>&page=<?= $i ?>&lang=<?= urlencode($lang) ?>"><?= $i ?></a>
                </li>
              <?php endfor; ?>
            </ul>
          </nav>
